...
def gen_bdm(num_blocks, block_size):
    ...
    return np.kron(np.eye(num_blocks, dtype=int), 1-np.eye(block_size, dtype=int))
...
if __name__ == "__main__":
    ...
    gen_alist_file(am, "roc_"+str(num_blocks)+"_"+str(block_size)+"_add1.csv")
    gen_alist_file(dm, "roc_"+str(num_blocks)+"_"+str(block_size)+"_rm1.csv")
    
    am = np.zeros((num_blocks*block_size, num_blocks*block_size))
    dm = np.zeros((num_blocks*block_size, num_blocks*block_size))
    new_new_a = np.copy(new_a)
    for i in range(1, num_blocks):
        move_node(i*block_size-1, i-1, block_size, new_a, dm, am, new_new_a)
        
    move_node(block_size*num_blocks-1, num_blocks-1, block_size, new_a, dm, am, new_new_a)
    gen_alist_file(am, "roc_"+str(num_blocks)+"_"+str(block_size)+"_add2.csv")
    gen_alist_file(dm, "roc_"+str(num_blocks)+"_"+str(block_size)+"_rm2.csv")
    print a
    print
    print am
    print
