    ...
    //public:
        ...
        int64_t merge(int64_t u, int64_t v) // Merge communities u and v. Returns id of parent.
        {
            ...
            if(u != v)
            {
                ...
                if(parent == 23 && child == 46) cout << "############### " << get_adjacency(parent, parent) << ", " << get_adjacency(child, child) << endl;
        ...
        void split(int64_t u)
        {
            ...
            if(children[u].size()>0)
            {
                ...
                for(map<int64_t, int64_t>::iterator nbr_it=nbrs[child].begin(); nbr_it!=nbrs[child].end(); ++nbr_it)
                {
                    int64_t nbr = nbr_it->first;
                    int64_t wt = nbr_it->second;
                    add_adjacency(u, nbr, -wt, child, parents[u][0].first);
                }
                add_adjacency(u, u, -get_adjacency(child, child), active[child], parents[u][0].first); // Remove community internal edges
                active[child] = 1;
                //cout << "woohoo3" << endl;
            }
            time++;
