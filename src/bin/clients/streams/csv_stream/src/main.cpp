...
main(int argc, char *argv[])
{
  ...
  int header = 0;
  while(-1 != (opt = getopt(argc, argv, "i:d:t:h:o:x:y:n:s:p:a"))) {
    switch(opt) {
      case 'i': {
    ins_file = optarg;
    } break;
	...
	if(NULL == server) {
      ...
      case '?': {
	printf("Usage:    %s -i insert_file -d delete_file -t file_type (adjacency list/edge list) [-h no. of header lines] [-o start node offset] [-x batch_insertions] [-y batch_deletions] [-n num_batches] [-p port] [-a server_addr]\n", argv[0]);
    ...
    for(int64_t i=0;i<num_batches;i++)
    {
        ...
        if(i_eof == 0)
        {
            for(int64_t j=0;j<batch_ins;j++)
            {
                ...
                int64_t dest = atol(dest_str.c_str())-offset;
                insertion->set_destination(dest);
                std::ostringstream dest_stream;
                dest_stream << dest;
                dest_str = dest_stream.str();
                ...
                if(file_type == "el")
                {
                    ...
                    int64_t src = atol(src_str.c_str())-offset;
                    insertion->set_source(src);
